...
AnotherSSH 提供了完整的应用菜单栏，所有主要功能都可以通过菜单快速访问。

### 文件菜单

- **本地终端** (`Ctrl/Cmd+T`) - 打开本地 Shell 终端
- **新建连接** (`Ctrl/Cmd+N`) - 打开新建 SSH 连接对话框
- **新建分组** (`Ctrl/Cmd+Shift+N`) - 创建新的会话分组
- **导入配置** (`Ctrl/Cmd+I`) - 导入会话配置文件
- **导出配置** (`Ctrl/Cmd+E`) - 导出会话配置文件
- **设置** (`Ctrl/Cmd+,`) - 打开设置对话框
- **退出** - 退出应用
...
- **撤销/重做** - 标准编辑操作
- **剪切/复制/粘贴/全选** - 标准剪贴板操作
- **查找** (`Ctrl/Cmd+F`) - 在终端中搜索
- **清屏** (`Ctrl/Cmd+K`) - 清除当前终端内容

### 查看菜单

- **切换侧边栏** (`Ctrl/Cmd+B`) - 显示/隐藏左侧会话列表
- **放大** (`Ctrl/Cmd+=`) - 增大终端字体
- **缩小** (`Ctrl/Cmd+-`) - 减小终端字体
- **重置缩放** (`Ctrl/Cmd+0`) - 恢复默认字体大小
- **重新加载** - 刷新应用
- **切换开发者工具** - 打开/关闭调试工具
- **全屏** - 切换全屏模式
...
- **水平分屏** (`Ctrl/Cmd+Shift+D`) - 创建水平分屏
- **垂直分屏** (`Ctrl/Cmd+Shift+E`) - 创建垂直分屏
- **关闭分屏** (`Ctrl/Cmd+W`) - 关闭当前分屏
- **最小化/缩放/关闭** - 标准窗口操作
...
- **项目主页** - 访问 GitHub 项目页面
- **检查更新** - 检查应用更新
- **关于 AnotherSSH** - 显示应用版本和信息

### 使用技巧

- 所有菜单项都有对应的快捷键，熟练使用可以大幅提升效率
- macOS 用户：应用菜单（最左边）包含"关于"和"设置"选项
- Windows/Linux 用户："关于"在帮助菜单中，"设置"在文件菜单中
...
本地终端功能让你无需连接远程服务器，直接在应用内使用本地 Shell，方便进行本地操作和测试。

### 什么是本地终端？

本地终端是在你的电脑上运行的 Shell 环境：
- **macOS/Linux**：默认使用系统 Shell（zsh、bash 等）
...
**3. 文件对比**
- 本地终端：`cat local.conf`
- SSH 终端：`cat /etc/app.conf`
- 对比配置差异

**4. 日志监控**
- 本地终端：`tail -f local.log`
- SSH 终端：`tail -f /var/log/app.log`
- 同时监控本地和远程日志
...
**Windows 用户**：
- 默认使用 cmd.exe
- 如需使用 PowerShell，可以在 cmd 中输入 `powershell`
- 支持中文路径和文件名

**macOS/Linux 用户**：
- 自动使用系统默认 Shell（通常是 zsh 或 bash）
- 支持所有 Shell 特性（颜色、自动补全等）
- 可以使用 `chsh` 命令更改默认 Shell

**性能**：
- 本地终端使用 PTY（伪终端），性能接近原生终端
- 支持大量输出和快速响应
- 不受网络影响
...
**方式 1：顶级分组**
1. 点击会话列表上方的 **"新建分组"** 按钮（或使用菜单 `Ctrl/Cmd+Shift+N`）
...
- **重命名分组**：鼠标悬停在分组上，点击 "重命名" 按钮
- **删除分组**：鼠标悬停在分组上，点击 "删除" 按钮（会话会移到默认分组）
...
**Q: 本地终端打开后是空白的（Windows）**

A: 这是 Windows cmd.exe 的特性，需要按一下回车键显示提示符。应用会自动发送回车，如果仍然空白：
1. 手动按一下回车键
2. 输入 `echo test` 测试是否正常
3. 如果完全无响应，关闭后重新打开
...
A: 可能原因：
1. 终端未获得焦点，点击终端区域
2. 等待 1-2 秒让 cmd.exe 完全启动
3. 按一下回车键激活输入

**Q: 本地终端显示的 Shell 不是我想要的（macOS/Linux）**

A: 本地终端使用系统默认 Shell。更改方法：
```bash
# 查看当前 Shell
echo $SHELL

# 更改默认 Shell（需要重启应用）
chsh -s /bin/zsh   # 改为 zsh
chsh -s /bin/bash  # 改为 bash
```

**Q: Windows 本地终端想用 PowerShell**

A: 两种方法：
1. 在 cmd 中输入 `powershell` 切换
2. 或者在 cmd 中输入 `pwsh`（如果安装了 PowerShell Core）

**Q: 本地终端和系统终端有什么区别？**

A: 功能基本相同，但有一些差异：
- 本地终端集成在应用内，无需切换窗口
- 支持应用的所有终端功能（搜索、分屏、字体调整等）
- 可以和 SSH 终端混合使用
- 性能接近原生终端
